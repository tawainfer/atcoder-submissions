// https://atcoder.jp/contests/abc273/submissions/35683155
...
int main() {
  ...
  fore(z, a) {
    if(z == ma) {
      ...
      ans[distance(upper_bound(all(search), z), search.end())]++;
      // cout << distance(s.upper_bound(z), s.end()) << " " << ans[distance(s.upper_bound(z), s.end())] << endl;
    }

    //cnt[z]--;
    //if(cnt[z] == 0) s.erase(z);
  }
