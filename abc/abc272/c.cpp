// https://atcoder.jp/contests/abc272/submissions/35477602
...
int main() {
  ...
  vector<int> o, e;
  if(odd < 2 && even < 2) cout << -1;
  else {
    sort(all(a));
    reverse(all(a));

    fore(z, a) {
      if(z % 2) o.push_back(z);
      else e.push_back(z);
    }

    if(o.size() < 2) cout << e[0] + e[1];
    else if(e.size() < 2) cout << o[0] + o[1];
    else cout << max(e[0] + e[1], o[0] + o[1]);
  }
}
