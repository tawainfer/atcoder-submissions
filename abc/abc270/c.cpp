// https://atcoder.jp/contests/abc270/submissions/35149971
...
int main() {
  ...
  while(!dq.empty()) {
    ...
    bool check = false;
    int d = 1e9;
    if(g[b].back() != -1) d = distance(g[b].begin(), upper_bound(all(g[b]), 0));

    rep(i, d, (int)g[b].size()) {
      if(!seen[g[b][i]]) {
        dq.push_back(g[b][i]);
        g[b][i] = 0;
        check = true;
