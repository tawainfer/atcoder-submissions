// https://atcoder.jp/contests/abc211/submissions/35082209
...
int main() {
  ...
  rep(j, 0, (int)s.size() + 1) dp[0][j] = 1;
  rep(i, 1, 9) {
    rep(j, 1, (int)s.size() + 1) {
      dp[i][j] += dp[i][j - 1];
      if(i == 1 && s[j - 1] == 'c') dp[i][j] += dp[i - 1][j - 1];
      if(i == 2 && s[j - 1] == 'h') dp[i][j] += dp[i - 1][j - 1];
      if(i == 3 && s[j - 1] == 'o') dp[i][j] += dp[i - 1][j - 1];
      if(i == 4 && s[j - 1] == 'k') dp[i][j] += dp[i - 1][j - 1];
      if(i == 5 && s[j - 1] == 'u') dp[i][j] += dp[i - 1][j - 1];
      if(i == 6 && s[j - 1] == 'd') dp[i][j] += dp[i - 1][j - 1];
      if(i == 7 && s[j - 1] == 'a') dp[i][j] += dp[i - 1][j - 1];
      if(i == 8 && s[j - 1] == 'i') dp[i][j] += dp[i - 1][j - 1];
      dp[i][j] %= mod;
    }
  }
