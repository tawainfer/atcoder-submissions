// https://atcoder.jp/contests/abc283/submissions/37506250
...
int main() {
  ...
  vector<int> l;
  map<char, int> m;
  vector<char> stock[300009];
  
  rep(i, 0, n) {
    if(s[i] == '(') {
      l.push_back(i);
    } else if(s[i] == ')') {
      for(auto &z : stock[l.back()]) m[z]--;
      l.pop_back();
    } else {
      if(m[s[i]] > 0) {
        cout << "No";
      ...
      m[s[i]]++;
      if(l.size() > 0) stock[l.back()].push_back(s[i]);
      else stock[300005].push_back(s[i]);
    }
  }
