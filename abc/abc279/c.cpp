// https://atcoder.jp/contests/abc279/submissions/36811740
...
int main() {
  ...
  rep(i, 0, h) {
    cin >> x;
    rep(j, 0, w) { sstr[j] += x[j]; }
  }

  rep(i, 0, h) {
    cin >> x;
    rep(j, 0, w) { tstr[j] += x[j]; }
  }

  rep(i, 0, w) {
    if(ma.find(sstr[i]) == ma.end()) ma[sstr[i]] = 0;
    if(mb.find(sstr[i]) == mb.end()) mb[sstr[i]] = 0;
    if(ma.find(tstr[i]) == ma.end()) ma[tstr[i]] = 0;
    if(mb.find(tstr[i]) == mb.end()) mb[tstr[i]] = 0;
  }
