// https://atcoder.jp/contests/arc149/submissions/35368319
...
int main() {
  ...
  ll n, mod;
  cin >> n >> mod;
  pair<ll, ll> ans = make_pair(0, 0); // len, num
  ...
  reps(i, 1, n) {
    reps(j, 1, 9) {
      mo[j] = mo[j] * 10 + j;
      mo[j] %= mod;
      if(!mo[j]) ans = make_pair(i, j);
    }
  }

  if(ans.first) {
    while(ans.first--) cout << (char)(ans.second + 48);
  } else cout << -1;
