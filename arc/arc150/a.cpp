// https://atcoder.jp/contests/arc150/submissions/35550529
...
int main() {
  ...
  while(cin >> n >> k >> s) {
    ...
    rep(i, 0, n + 1) {
      if(s[i] == '0') {
        if(streak > 0) ptn.push_back(s.substr(start, streak));
        streak = 0;
      } else {
        if(streak == 0) start = i;
        streak++;
